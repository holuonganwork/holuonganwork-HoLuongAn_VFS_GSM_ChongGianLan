const paths = {
  ...
  process: '<path d="M4 6h16M4 12h16M4 18h16"/><circle cx="8" cy="6" r="2"/><circle cx="16" cy="12" r="2"/><circle cx="10" cy="18" r="2"/>',
  ...
  branch: '<circle cx="6" cy="4" r="2"/><circle cx="18" cy="6" r="2"/><circle cx="6" cy="20" r="2"/><path d="M6 6v12m0-6h6a6 6 0 0 0 6-4"/>',
...
export const number = value => value == null ? '—' : new Intl.NumberFormat('vi-VN').format(value);
export const date = value => value ? new Intl.DateTimeFormat('vi-VN', {dateStyle:'short', timeStyle:'short'}).format(new Date(value)) : 'Chưa có dữ liệu';
export const labels = {
  gps_spoofing: 'Giả mạo GPS', repeated_trips: 'Cuốc xe lặp lại', shared_device: 'Dùng chung thiết bị', promotion_abuse: 'Lạm dụng khuyến mãi',
  detected: 'Chờ review', under_review: 'Đang review', confirmed_fraud: 'Xác nhận gian lận', dismissed: 'Đã loại trừ',
  awaiting_driver_explanation: 'Chờ review (cũ)', driver_responded: 'Chờ review (cũ)',
  auto_clear: 'Auto Clear', auto_fraud: 'Auto Fraud', human_review: 'Human Review',
  active: 'Hoạt động', medium: 'Trung bình', high: 'Cao', low: 'Thấp', critical: 'Nghiêm trọng', unknown: 'Chưa xác định',
};
export function badge(value) {
  const tone = ['auto_clear','dismissed','active'].includes(value) ? 'green' : ['auto_fraud','confirmed_fraud','high','critical'].includes(value) ? 'red' : 'amber';
...
export function writeLocal(key, value) {
  ...
  catch { toast('Trình duyệt không cho phép lưu cài đặt.'); return false; }
}
...
export function empty(message = 'Chưa có dữ liệu', detail = 'Dữ liệu sẽ xuất hiện sau khi hệ thống ghi nhận kết quả.') {
